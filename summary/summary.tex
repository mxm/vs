...
\item concurrent read, concurrent write
	...
	\end{itemize}
\end{itemize}
\textbf{(n,m)-PRAM} modelliert Parallelrechner mit n Prozessoren und m Speicherworten, ähnlich einem shared memory MIMD System.
\subsection{LogP}
\begin{tabular}{lll}
...
\subsection{Threads}
Ein \textbf{Thread} ist Betriebssystemabstraktion einer Aktivität. Threads sind nicht voneinander isoliert und besitzen nur einen Kontext.
\begin{itemize}
	...
	\item Aber: Threads im selben Prozess sind nicht gegeneinander geschützt
\end{itemize}
...
\begin{itemize}
	\item UMA (Uniform Memory Access): zentraler Speicher mit immer gleichen Zugriffszeiten - Einsockel-Multicore-Systeme $\rightarrow$ PC, Laptop
	\item NUMA (Non-Uniform ...): Jeder Prozessor hat eigenen Speicher, gewährt anderen Prozessoren aber Zugriff - Cray T3E (früher), Cray XT5, 6
	\item ccNUMA (cache coherent NUMA): alle Mehrsockel-Multicore-Systeme
	\item DM: Cluster
\end{itemize}
...
C = $h_{1}^{c_{1}} \cup h_{2}^{c_{2}} \cup$ ... $\cup h_{N}^{c_{N}}$ \\ \\
...
Real-Time Garbage Collection, wobei jeder Prozess seinen eigenen Heap besitzt.
...
\begin{itemize}
	\item Vollständigkeit: Wenn ein Prozess abstürzt, sendet er keine Heartbeats mehr. Der Ausfalldetektor erhält dann wegen der pp2p-Links keine Heartbeats mehr und er erkennt P als abgestürzt.
...
\begin{itemize}
	...
	\item Verteilte Systeme legen den Fokus auf \underline{Verfügbarkeit}. Konsistenz ist oft unklar. Das Transaktionssystem löst zwei zentrale Probleme: \textbf{Ausfalltransparenz} (wie oben) und zusätzlich \textbf{Nebenläufigkeitstransparenz}
\end{itemize}
...
\end{itemize}
\textbf{Problem:} Kaskadierende Abbrüche durch dirty read \\
